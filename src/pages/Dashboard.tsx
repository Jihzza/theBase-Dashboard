  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: logRows, error: logError }, { data: todoRows, error: todoError }, { data: cronRows }] =
        await Promise.all([
          sb.from("logs").select("*").order("finished_at", { ascending: false }).limit(50),
          sb
            .from("todos")
            .select("*")
  ...
  return (
    <AppShell>
      ...
      <AppLayout title="Dashboard">
        <SectionHeader
          title="Overview"
          subtitle="Today’s tasks, recent logs, and daily signals."
        />
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.4fr_1fr]">
                  ...
                  {upcomingLogs.map((l) => (
                    <details key={l.id} className="border-b border-border last:border-b-0">
                      <summary className="cursor-pointer list-none grid grid-cols-12 gap-3 px-5 py-4 hover:bg-surfaceAlt">
                        <div className="col-span-4 text-sm text-ink">{fmt(l.finished_at ?? l.timestamp)}</div>
          ...
          <Panel title="Quick actions" subtitle="Jump to key areas">
            <div className="grid gap-2 text-sm">
              ...
              <Link to="/app/logs" className="rounded-xl border border-border bg-surface px-4 py-3 hover-soft">
                View logs
              </Link>
              <Link to="/app/cron" className="rounded-xl border border-border bg-surface px-4 py-3 hover-soft">
                Manage cron jobs
              </Link>
            </div>
          </Panel>
